...
- **CLI Enhancements**
    - [ ] Refactor `cmd/sudoku/main.go` to use the standard `flag` package for robust argument parsing. (Ref: specs/input-handling.md 1.1)
    - [ ] Support operational flags: `-file` (input file), `--brute-force` (toggle fallback), `--log-level` (info, debug, trace), and `--test` (regression mode). (Ref: specs/input-handling.md 1.2, 1.3; specs/output-rendering.md 2.1; specs/testing-regression.md 1.2)
- **Hodoku Library Format Support**
    - [ ] Update `internal/puzzle/reader.go` to support the Hodoku Library Format: `:technique:candidates:givens:deleted:eliminations:placements:extra`. (Ref: specs/input-handling.md 3.1)
    - [ ] Correctly distinguish between givens and placed values (prefixed with `+`) during parsing. (Ref: specs/input-handling.md 3.2)
    - [ ] Implement support for the `<deleted candidates>` field to initialize board states with specific eliminations for testing. (Ref: specs/input-handling.md 3.3)
- **Test Setup Utility**
    - [ ] Add a helper function `FromHodokuString(s string)` in `internal/puzzle` to simplify puzzle initialization in unit tests and the regression runner. (Ref: specs/testing-regression.md 3.1, 3.2)
...
- **Regression Test Runner**
    - [ ] Implement a dedicated regression runner to verify deductive techniques against real-world test cases. (Ref: specs/testing-regression.md 1.1)
    - [ ] Add verification logic to ensure the solver performs the *exact* eliminations or placements specified in a Hodoku test case. (Ref: specs/testing-regression.md 2.2, 2.3)
    - [ ] Report detailed summary statistics including total tests, pass/fail counts, and execution time. (Ref: specs/testing-regression.md 1.3)
...
- [ ] **Full House**: Implement as a distinct technique with higher priority than Hidden Single. (Ref: specs/deductive-techniques.md 1.1)
